<x-app-layout>
    <div class="container mx-auto p-4">
        ...
        <section class="py-8 container px-4 mx-auto">
            <div class="bg-white p-4 mb-4 rounded-xl ">
                <form action="{{ auth()->user()->isAdmin() ?  route('offers.index') : route('offers.my') }}" method="GET"
                    ...
                    <div class="px-8 py-2 bg-white border border-gray-200 rounded-3xl">
                        <select name="status"
                            ...
                            <option {{ request()->query('status') === $status ? 'selected' : '' }} value="{{ $status }}">
                                {{ $status }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="px-8 py-2 bg-white border border-gray-200 rounded-3xl">
                        <select class="w-full bg-transparent text-md text-gray-900 placeholder-gray-200 font-bold outline-none"
                            name="location">
                            <option disabled selected>Select location...</option>
                            @foreach($locations as $location)
                            <option {{ request()->query('location') == $location->id ? 'selected' : '' }}
                    ...
                    <div class="px-8 py-2 bg-white border border-gray-200 rounded-3xl">
                        <select class="w-full bg-transparent text-md text-gray-900 placeholder-gray-200 font-bold outline-none"
                            ...
                            <option {{ request()->query('category') == $category->id ? 'selected' : '' }}
                    ...
                    <div class="bg-white">
                        <input
                            ...
                            type="text" name="title" placeholder="Search by title..." value="{{ request()->query('title') }}">
                    </div>
                    <div>
                        <button type="submit"
            ...
            @if ($offers->count() <= 0) <div class="mb-16 bg-white border border-gray-100 rounded-xl">
        ...
        <table class="table-auto min-w-full">
            <thead>
                <tr class="bg-gray-200 py-10 h-20">
                    ...
                    @endforeach
                    <td class="relative px-2 border-b border-gray-100">
                        <div class="flex justify-center items-center pl-4 text-sm font-heading font-semibold uppercase">
                            Action
                        </div>
                    </td>
                </tr>
            ...
            <tbody>
                ...
                <tr class="border-b border-gray-100">
                    <td class="p-0">
                        <div class="flex break-words items-center pl-4">
                            <img class="w-8 h-8 rounded-full object-cover" src="{{ asset($offer->author->image_url) }}"
                                alt="User">
                    ...
                    <td class="p-0">
                        <a href="{{ route('offers.show', $offer) }}" class="flex break-words items-center pl-4">
                    ...
                    <td class="p-0">
                        <div class="flex break-words items-center p-5">
                            <span class="text-darkBlueGray-400 font-heading">{{ getTitles($offer->categories) }}</span>
                        </div>
                    </td>
                    <td class="p-0">
                        <div class="flex break-words items-center p-5">
                            <span class="text-darkBlueGray-400 font-heading">{{ getTitles($offer->locations) }}</span>
                        </div>
                    </td>
                    <td class="p-0">
                    ...
                    <td class="p-0">
                        <div class="flex items-center justify-center p-5">
                            ...
                            <button data-delete-route="{{ route('offers.destroy', $offer->id) }}"
                              data-csrf-token="{{ csrf_token() }}"  class="delete-item-btn inline-flex w-8 h-8 items-center justify-center bg-red-500 hover:bg-red-600 rounded-2xl">
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="20" height="20">
                                    <path fill="none" d="M0 0h24v24H0z" />
                                    <path
